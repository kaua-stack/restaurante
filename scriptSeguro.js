...
document.addEventListener('DOMContentLoaded', () => {
    ...
    `;
        ...
        // ---- NOVO: Adicionar produto ao carrinho ----
        const addBtn = modal.querySelector('.add-to-cart-btn');
        addBtn.addEventListener('click', () => {
            const qtd = parseInt(modal.querySelector('#quantidade').textContent);
            if (qtd <= 0) return;

            adicionarAoCarrinho(produto, qtd);
            closeModal(); // Fecha modal após adicionar
        });
    }
...
function atualizarCarrinho() {
    const containerDesktop = document.getElementById('itens-sacola');
    const containerMobile = document.getElementById('itens-sacola-mobile');
    const totalDesktop = document.getElementById('total-sacola');
    const totalMobile = document.getElementById('total-sacola-mobile');
    ...
    carrinho.forEach(item => {
        ...
        `;
        containerDesktop.innerHTML += itemHTML;
        containerMobile.innerHTML += itemHTML;
        total += item.preco * item.quantidade;
    });

    totalDesktop.textContent = `Total: R$ ${total.toFixed(2)}`;
    totalMobile.textContent = `Total: R$ ${total.toFixed(2)}`;

    document.getElementById('cartCount').textContent = carrinho.reduce((sum, i) => sum + i.quantidade, 0);
}

function openModal() {
    if (window.innerWidth <= 748) {
        document.getElementById('modalSacolaMobile').style.display = 'flex';
        document.body.style.overflow = 'hidden';
    }
}

function closeModal() {
    document.getElementById('modalSacolaMobile').style.display = 'none';
    document.body.style.overflow = '';
}
